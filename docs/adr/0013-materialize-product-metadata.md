...
The two explicit jobs make intent visible: `update-product-metadata` handles a bounded set of new ASINs; `refresh-product-metadata` handles the whole account catalog. Both batch at Amazon's 300-ASIN request limit, fail fast when the profile exposes no matching inventory, and expose aggregate Event Stream and per-request API metrics.

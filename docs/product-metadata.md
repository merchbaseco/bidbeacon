...
The dashboard Account Data card shows Product Metadata coverage over distinct advertised ASINs. Its row is green when coverage is complete and neutral while titles remain unresolved. Manual refresh skips ASINs updated within the one-week freshness SLA and throttles overlapping account requests. Product metadata writes emit `product-metadata:updated`, which invalidates only the focused coverage query.

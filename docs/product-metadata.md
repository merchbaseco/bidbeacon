...
The dashboard Account Data card shows Product Metadata coverage over distinct advertised ASINs. Its row is green when coverage is complete, neutral while titles remain unresolved, and shows a spinner only while that account's metadata refresh is running. Every attempted ASIN records `last_fetched_at`, including empty Amazon results, and every caller skips ASINs fetched within the one-week freshness SLA. Product metadata jobs emit `product-metadata:updated` after reaching their final state, which invalidates only the focused coverage query.

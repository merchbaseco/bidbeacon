...
- `segments.hour`: account-local hour from `0` through `23`; available for Ad-group, Ad, and Product Search and requires `segments.date`
...
- `target` and `change_event`: cataloged but not implemented in this Search slice

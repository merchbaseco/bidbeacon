 ...
 * and Compose interpolates `${VAR}` from there. No `--env-file`, and no
...
const hugeiconsTokenName = 'MERCHBASE_HUGEICONS_LICENSE_KEY';
const projectName = 'bidbeacon';

const dryRun = process.argv.includes('--dry-run');

// A leftover .env in the deploy checkout silently poisons the whole resolution:
// varlock loads it at higher precedence than the schema, so its stale values win,
// its `$` sequences are parsed as ref() expressions, and — once one item fails to
// parse — every op() reference in the file reports "Unable to authenticate with
// 1Password". That is how a superseded .env took production down after the
// migration: the database password arrived truncated at its first `$`.
if (existsSync('.env')) {
    console.error(
...
const varlockRun = (args: string[], env = environment) => spawnSync('bunx', ['varlock', 'run', '--', ...args], { env, stdio: 'inherit' });
...
// from the process environment. On the Actions runner the workflow supplies the
// GitHub token from `github.token`, which needs no 1Password access at all.
//
// The operator fallback resolves them from the Development vault, NOT the
...
for (const name of [installTokenName, hugeiconsTokenName]) {
    if (environment[name]) {
        continue;
    }

    const token = printenv(name, { VARLOCK_ENV: 'development', BIDBEACON_RESOLVE_INSTALL_TOKENS: 'true' });
    if (token) {
...
const argNames = [...readFileSync('Dockerfile', 'utf8').matchAll(/^ARG\s+([A-Z][A-Z0-9_]*)/gmu)].map(match => match[1]);

for (const name of argNames) {
    buildEnvironment[name] = printenv(name);
}

const composeArgs = ['-p', projectName, '-f', 'compose.yml'];
...
if (dryRun) {
    const rendered = spawnSync('bunx', ['varlock', 'run', '--', 'docker', 'compose', ...composeArgs, 'config'], {
    ...
    console.log(`Dry run OK: schema resolved and Compose rendered ${services.length} services (${services.join(', ')}).`);
    process.exit(0);
}

console.log('Building images. Compose warnings about unset runtime variables are expected here — the build only consumes build arguments and the install tokens.');

const build = spawnSync('docker', ['compose', ...composeArgs, 'build'], { env: buildEnvironment, stdio: 'inherit' });
if (build.status !== 0) {
    console.error('Image build failed; deploy not attempted.');
    process.exit(build.status ?? 1);
}

// Migrations run automatically on server startup (src/db/migrate.ts), so `up`
// is the whole delivery step.

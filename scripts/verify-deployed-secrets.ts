 ...
 *
 * The inspect output is read as JSON rather than line-split text: BidBeacon
 * delivers a multi-line PEM (MERCHBASE_CLERK_JWT_KEY), and a naive line split
 * turns each continuation line into a bogus "delivered name".
 *
 ...
 * server container.
 */
const serverContainer = 'bidbeacon-server';
const workerContainer = 'bidbeacon-worker';

// Names the runtime image sets for itself rather than receiving from the
// schema; they are not part of the environment contract.
const imageProvidedNames = new Set(['PATH', 'NODE_ENV', 'NODE_VERSION', 'YARN_VERSION', 'HOSTNAME', 'HOME', 'TERM']);
...
const deliveredNamesFor = (container: string): string[] => {
    ...
    return entries.map(entry => entry.slice(0, entry.indexOf('='))).filter(name => name.length > 0 && !imageProvidedNames.has(name));
};

const failures: string[] = [];
const summaries: string[] = [];

for (const container of [serverContainer, workerContainer]) {
    ...
    // the worker deliberately receives a narrower set (no webhook secret, no
    // MCP or dashboard configuration).
...
console.log(`Delivered environment matches the schema — ${summaries.join('; ')}; ${requiredInProduction.size} production-required items verified on ${serverContainer}.`);

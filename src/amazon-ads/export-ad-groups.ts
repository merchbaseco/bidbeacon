/**
 * Amazon Ads API - Export Ad Groups Bridge
 * Handles creating file-based exports of ad groups via the Amazon Ads API
 */
...
export type ExportAdGroupsRequest = z.infer<typeof baseExportRequestSchema>;
export type ExportAdGroupsResponse = z.infer<typeof exportResponseSchema>;

export interface ExportAdGroupsOptions {
...
/**
 * Creates a file-based export of ad groups via the Amazon Ads API
...
export async function exportAdGroups(options: ExportAdGroupsOptions, region: ApiRegion = 'na'): Promise<ExportAdGroupsResponse> {
    return withTracking({ apiName: 'exportAdGroups', region }, async () => {
        ...
        const url = `${baseUrl}/adGroups/export`;

        // Build request body
        const requestBody: ExportAdGroupsRequest = {};
        ...
        if (!response.ok) {
            ...
            const error = new Error(`Failed to export ad groups: ${response.status} ${response.statusText}. ${errorText}`);

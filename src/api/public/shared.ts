...
export const assertAccountAccess = (ctx: ApiContext, config: PublicConfig) => {
    ctx.assertAccountAccess(config.accountId);
};

export const resolveAccountContext = async (config: PublicConfig): Promise<AccountContext> => {
...
export const listCampaigns = async (config: PublicConfig, options?: ListOptions): Promise<CampaignShape[]> => {
...
export const getCampaign = async (config: PublicConfig, campaignId: string): Promise<CampaignShape> => {
    const countryCode = normalizeCountryCode(config.countryCode);
    const [row] = await db
        .select({
            campaignId: campaign.campaignId,
...
export const listAdGroups = async (config: PublicConfig, options?: ListOptions): Promise<AdGroupShape[]> => {
    const countryCode = normalizeCountryCode(config.countryCode);
    const stateFilter = resolveListState(options?.state);
    const limit = options?.limit ?? DEFAULT_LIST_LIMIT;
    const offset = options?.offset ?? 0;
...
export const getAdGroup = async (config: PublicConfig, adGroupId: string): Promise<AdGroupShape> => {
    const countryCode = normalizeCountryCode(config.countryCode);
    const [row] = await db
        .select({
            adGroupId: adGroup.adGroupId,
...
export const listAds = async (config: PublicConfig, options?: ListOptions): Promise<AdShape[]> => {
    const countryCode = normalizeCountryCode(config.countryCode);
    const stateFilter = resolveListState(options?.state);
    const limit = options?.limit ?? DEFAULT_LIST_LIMIT;
    const offset = options?.offset ?? 0;
...
export const getAd = async (config: PublicConfig, adId: string): Promise<AdShape> => {
    const countryCode = normalizeCountryCode(config.countryCode);
    const [row] = await db
        .select({
            adId: ad.adId,
...
export const getTarget = async (config: PublicConfig, targetId: string): Promise<TargetShape> => {
    const countryCode = normalizeCountryCode(config.countryCode);
    const [row] = await db
        .select({
            targetId: target.targetId,
...
export async function getMetricsTable(config: PublicConfig, dimension: 'campaign', options: MetricsTableRequest): Promise<MetricsTableResult['campaign']>;
export async function getMetricsTable(config: PublicConfig, dimension: 'adGroup', options: MetricsTableRequest): Promise<MetricsTableResult['adGroup']>;
export async function getMetricsTable(config: PublicConfig, dimension: 'ad', options: MetricsTableRequest): Promise<MetricsTableResult['ad']>;
export async function getMetricsTable(config: PublicConfig, dimension: 'target', options: MetricsTableRequest): Promise<MetricsTableResult['target']>;
export async function getMetricsTable(config: PublicConfig, dimension: MetricsDimension, options: MetricsTableRequest): Promise<MetricsTableResult[MetricsDimension]> {
    const account = await resolveAdvertiserAccount(config);
...
const resolveEntityFilterIds = async (config: PublicConfig, dimension: MetricsDimension, scope: MetricsFilters, filters: MetricsFilterInput) => {
    const countryCode = normalizeCountryCode(config.countryCode);
...
const resolveAdvertiserAccount = async (config: PublicConfig) => {
    const countryCode = normalizeCountryCode(config.countryCode);
    if (countryCode) {

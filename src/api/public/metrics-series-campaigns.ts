import { metricsSeriesInputSchema, metricsSeriesOutputSchema } from '@/api/public/schemas';
import { apiProcedure } from '@/api/trpc';
import { assertAccountAccess, getMetricsSeries } from './shared';

export const metricsSeriesCampaigns = apiProcedure
    .input(metricsSeriesInputSchema)
    .output(metricsSeriesOutputSchema)
    .query(async ({ ctx, input }) => {
        assertAccountAccess(ctx, input.config);
        return getMetricsSeries(input.config, 'campaign', {
            scope: {

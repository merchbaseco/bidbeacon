...
const parseDateString = (value: string) => {
    ...
    if (!Number.isInteger(monthNumber) || !Number.isInteger(dayNumber) || !Number.isInteger(yearNumber)) return null;
    ...
    if (date.getFullYear() !== yearNumber || date.getMonth() !== monthNumber - 1 || date.getDate() !== dayNumber) return null;
...
const buildSortExpression = (
    sortField: 'impressions' | 'clicks' | 'orders' | 'spend' | 'sales' | 'ctr' | 'cpc' | 'roas' | 'acos',
    metrics: ReturnType<typeof buildMetricExpressions>
) => {
    switch (sortField) {
        case 'impressions':

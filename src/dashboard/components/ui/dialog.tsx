...
function DialogClose({ children, ...props }: DialogPrimitive.Close.Props) {
    if (isValidElement(children)) {
        return <DialogPrimitive.Close data-slot="dialog-close" {...props} render={renderProps => cloneElement(children, renderProps)} />;
    }
...
function DialogViewport({ className, ...props }: DialogPrimitive.Viewport.Props) {
    return (
        <DialogPrimitive.Viewport
            className={cn('fixed inset-0 z-50 grid grid-rows-[1fr_auto] justify-items-center pt-12 sm:grid-rows-[1fr_auto_3fr] sm:p-4', className)}
...
function DialogHeader({ className, ...props }: React.ComponentProps<'div'>) {
    return <div className={cn('flex flex-col gap-2 p-6 in-[[data-slot=dialog-popup]:has([data-slot=dialog-panel])]:pb-3 max-sm:pb-4', className)} data-slot="dialog-header" {...props} />;
}
...
function DialogTitle({ className, ...props }: DialogPrimitive.Title.Props) {
    return <DialogPrimitive.Title className={cn('font-heading text-xl leading-none', className)} data-slot="dialog-title" {...props} />;
}

...
function ProgressTrack({ className, ...props }: ProgressPrimitive.Track.Props) {
    return <ProgressPrimitive.Track className={cn('block h-1.5 w-full overflow-hidden rounded-full bg-input', className)} data-slot="progress-track" {...props} />;
}
...
function ProgressValue({ className, ...props }: ProgressPrimitive.Value.Props) {
    return <ProgressPrimitive.Value className={cn('text-sm tabular-nums', className)} data-slot="progress-value" {...props} />;
}

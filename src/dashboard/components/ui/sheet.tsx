            ...
            <SheetViewport inset={inset} side={side}>
                <SheetPrimitive.Popup
                    className={cn(
                        ...
                        inset && 'before:hidden sm:rounded-2xl sm:border sm:before:rounded-[calc(var(--radius-2xl)-1px)] sm:**:data-[slot=sheet-footer]:rounded-b-xl',
                        className
                    )}
...
function SheetHeader({ className, ...props }: React.ComponentProps<'div'>) {
    return <div className={cn('flex flex-col gap-2 p-6 in-[[data-slot=sheet-popup]:has([data-slot=sheet-panel])]:pb-3 max-sm:pb-4', className)} data-slot="sheet-header" {...props} />;
}
...
function SheetTitle({ className, ...props }: SheetPrimitive.Title.Props) {
    return <SheetPrimitive.Title className={cn('font-heading text-xl leading-none', className)} data-slot="sheet-title" {...props} />;
}
...
function SheetPanel({ className, scrollFade = true, ...props }: React.ComponentProps<'div'> & { scrollFade?: boolean }) {
    return (

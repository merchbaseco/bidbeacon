...
const inputGroupAddonVariants = cva(
    "flex h-auto cursor-text select-none appearance-none items-center justify-center gap-2 border-0 bg-transparent p-0 [&>kbd]:rounded-[calc(var(--radius)-5px)] [&>svg:not([class*='size-'])]:size-4 not-has-[button]:**:[svg]:opacity-72",
    {
        defaultVariants: {
            align: 'inline-start',
        },
...
function InputGroupAddon({ className, align = 'inline-start', ...props }: ComponentProps<'button'> & VariantProps<typeof inputGroupAddonVariants>) {
    return (
        <button
...
function InputGroupText({ className, ...props }: ComponentProps<'span'>) {
    return <span className={cn("flex items-center gap-2 text-muted-foreground [&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none", className)} {...props} />;
}

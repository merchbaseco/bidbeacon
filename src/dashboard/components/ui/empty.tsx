...
function Empty({ className, ...props }: React.ComponentProps<'div'>) {
    return (
        <div className={cn('flex min-w-0 flex-1 flex-col items-center justify-center gap-6 text-balance rounded-xl border-dashed p-6 text-center md:p-12', className)} data-slot="empty" {...props} />
    );
}
        ...
        <div className={cn('relative mb-6', className)} data-slot="empty-media" data-variant={variant} {...props}>
            {variant === 'icon' && (
                <>
                    <div
                        aria-hidden="true"
                        className={cn(emptyMediaVariants({ className, variant }), 'pointer-events-none absolute bottom-px origin-bottom-left -translate-x-0.5 -rotate-10 scale-84 shadow-none')}
                    />
                    <div
                        aria-hidden="true"
                        className={cn(emptyMediaVariants({ className, variant }), 'pointer-events-none absolute bottom-px origin-bottom-right translate-x-0.5 rotate-10 scale-84 shadow-none')}
                    />
                </>
            )}

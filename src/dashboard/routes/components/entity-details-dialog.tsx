...
const EntityDetailsDialog = ({ open, onOpenChange, row, accountId }: { open: boolean; onOpenChange: (open: boolean) => void; row: PerformanceTableRow | null; accountId: string }) => {
    ...
    const updateBid = api.ads.targets.updateBid.useMutation({
        onSuccess: result => {
            ...
            utils.entityHistory.latestTargetBidChange.invalidate({ accountId, targetId: result.targetId });
    ...
    const latestTargetBidChangeQuery = api.entityHistory.latestTargetBidChange.useQuery(
        {
            accountId,
    ...
    const latestTargetBidChange = latestTargetBidChangeQuery.data ?? null;

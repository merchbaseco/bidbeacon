...
async function hasPerformanceData(
    accountId: string,
    windowStart: Date,
    windowEnd: Date,
    ...
    const record = await db.query.performance.findFirst({
        where: and(
            eq(performance.accountId, accountId),
            eq(performance.aggregation, aggregation),
            gte(performance.date, windowStart),
            lt(performance.date, windowEnd)
        ),
    ...
    await db
        ...
        .onConflictDoUpdate({
            target: [
                ...
                reportDatasetMetadata.timestamp,
                reportDatasetMetadata.aggregation,
            ],
            set: {
                status,
                lastRefreshed,
...
async function updateForWindow(
    accountId: string,
    windowStart: Date,
    aggregation: 'hourly' | 'daily'
): Promise<void> {
    const windowEnd =
        aggregation === 'hourly' ? utcAddHours(windowStart, 1) : utcAddDays(windowStart, 1);
...
async function backfillMissingMetadata(
    ...
    const earliestPeriodStart =
        aggregation === 'hourly'
            ? utcSubtractHours(currentPeriodStart, retentionDays * 24)
            : utcSubtractDays(currentPeriodStart, retentionDays);

    // Iterate through all periods in the retention window
    const periods =
        aggregation === 'hourly'
            ? enumerateUtcHours(earliestPeriodStart, currentPeriodStart)
            : enumerateUtcDays(earliestPeriodStart, currentPeriodStart);

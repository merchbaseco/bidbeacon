...
const TEST_ARM_KEY = 'pk_test_YmlkYmVhY29uLXRlc3QuY2xlcmsuYWNjb3VudHMuZGV2JA';
...
const describeKey = (key: string) => {
    ...
    return {
        ...
        isBase64: Buffer.from(decoded, 'utf8').toString('base64').replace(base64PaddingPattern, '') === payload.replace(base64PaddingPattern, ''),
        decoded,
...
describe('committed Clerk publishable keys', () => {
    it('are decodable in every lifecycle arm, including the fake test one', () => {
    ...
    it('keeps the test arm a fake domain rather than a real instance', () => {
        expect(schema).toContain(TEST_ARM_KEY);
        expect(describeKey(TEST_ARM_KEY).decoded).toBe('bidbeacon-test.clerk.accounts.dev$');
    });
});

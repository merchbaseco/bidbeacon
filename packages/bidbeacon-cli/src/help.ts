...
const TOPICS: Record<HelpTopicKey, HelpTopic> = {
    ...
    config: {
        ...
        commands: [
            ...
            { left: 'set range <value>', right: 'Set default range (today|yesterday|7d|30d|YYYY-MM-DD..YYYY-MM-DD)' },
        ],
    ...
    metrics: {
        ...
        notes: [
            'Common flags: --metrics <keys> --range <range> --filter <expr> (repeatable) --search <text> --state <value>',
    ...
    'metrics series': {
        ...
        options: [
            ...
            { left: '--range <range>', right: 'Override configured range' },
            { left: '--bucket <value>', right: 'auto|hour|day|week|month|year' },
            { left: '--campaign <campaign_id>', right: 'Scope to a campaign (ad-groups/ads/targets)' },
            { left: '--ad-group <ad_group_id>', right: 'Scope to an ad group (ads/targets)' },
